...
with col3:
    st.metric(
        '⏱️ Odómetro Promedio',
        f'{avg_odometer:,.0f} mi' if not pd.isna(avg_odometer) else 'N/A'
    )
...
with visual_tab:
    ...
    if chart_type == '📈 Condición vs Año del Modelo':
        ...
        correlation = filtered_data[['odometer', 'price']].corr().iloc[0, 1]
        ...
        numeric_cols = filtered_data[['price', 'odometer', 'model_year', 'days_listed']].select_dtypes(include=['number']).dropna()
...
with data_tab:
    ...
    df_display = filtered_data[cols_to_show].sort_values(order_col, ascending=order_asc).reset_index(drop=True)
    ...
    st.dataframe(
        ...
        column_config={
            ...
            'odometer': st.column_config.NumberColumn('Odómetro (mi)', format='%,.0f'),
...
if show_advanced:
    ...
    with col_stats2:
        ...
        st.write(f'**Desviación Estándar:** {filtered_data["odometer"].std():,.0f} mi')
        st.write(f'**Rango Intercuartil (Q1-Q3):** {filtered_data["odometer"].quantile(0.25):,.0f} - {filtered_data["odometer"].quantile(0.75):,.0f} mi')
        st.write(f'**Skewness:** {filtered_data["odometer"].skew():.3f}')
